...
//		}
		...
		System.out.println("Time to count all original count: " + (endTime-start_time));
		...
		for (int clause_id = 0; clause_id < mln.getClauses().size(); clause_id++) {
			...
			for (int atom_id = 0; atom_id < formula.atoms.size(); atom_id++) {
				...
				for (int ground_id = 0; ground_id < atom.getNumberOfGroundings(); ground_id++) {

					gs.flipAtom(atom.symbol, ground_id);

					// Compute and cache the count of the formula with the current
					// ground atom flipped
					gs.noOfFalseGroundingsIncreased(clause_id).intValue();

					gs.unflipAtom(atom.symbol, ground_id);
				}
				
				endTime = System.currentTimeMillis();
				System.out.println("Time to count for "+atom.getNumberOfGroundings()+ " groundings for the symbol"+atom.symbol+" is: " + (endTime-start_time));
