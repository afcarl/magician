...
public class GurobiWmsSolver implements WeightedMaxSatSolver {
	...
	public void solve() {
		...
		try {
			...
			List<GRBVar> predicateVars = new ArrayList<GRBVar>(noVar);
			List<GRBVar> clauseVars = new ArrayList<GRBVar>(noSoftClauses);
			...
			for (int i = 0; i < noHardClauses; i++) {
				expr = new GRBLinExpr();
				double rhs = 1.0;
				
				int[] hardClause = hard_clauses.get(i);
				for (int j = 0; j < hardClause.length; j++) {
					
					int literal = hardClause[j];
					double sign = 1.0;
					if(literal < 0) {
						sign = -1.0;
						rhs = rhs - 1.0;
				...
				model.addConstr(expr, GRB.GREATER_EQUAL, rhs, "ch_"+i);
			}
			...
			for (int i = 0; i < predicateVars.size(); i++) {
				GRBVar grbVar = predicateVars.get(i);
				this.model[i] = (int) grbVar.get(GRB.DoubleAttr.Xn);
			}
